...
class IndentationRule {
    ...
    public constructor(config: IIndentationRuleConfig) {
        this.unIndentedLinePattern = this.createRegExp(config && config.unIndentedLinePattern)
        this.increaseIndentPattern = this.createRegExp(config && config.increaseIndentPattern)
        this.decreaseIndentPattern = this.createRegExp(config && config.decreaseIndentPattern)
        this.indentNextLinePattern = this.createRegExp(config && config.indentNextLinePattern)
    }
  ...
  public estimateIndentAction(validPreviousLine: string, currentLine: string): vscode.IndentAction {
...
function mergeLanguageConfiguration(a: ILanguageConfiguration, b: ILanguageConfiguration): ILanguageConfiguration {
    ...
    if (a.indentationRules) {
        mergedConfiguration.indentationRules = Object.assign({}, a.indentationRules, b.indentationRules)
    } else {
...
function getLanguageConfiguration(id: string): ILanguageConfiguration {
    ...
    for (const ext of vscode.extensions.all) {
        if (ext.packageJSON && ext.packageJSON.contributes && ext.packageJSON.contributes.languages) {
...
function estimateIndentAction(validPreviousLine: string, currentLine: string, languageConfiguration: ILanguageConfiguration): vscode.IndentAction {
    ...
    */
    ...
    if (validPreviousLine.length > 0 && currentLineWihtoutLeadingWhitespaces.length > 0) {
        for (const bracketConfig of bracketsArray) {
            const bracket = new BracketRule(bracketConfig)
            if (bracket.openRegExp.test(validPreviousLine) && bracket.closeRegExp.test(currentLineWihtoutLeadingWhitespaces)) {
 ...
 * @return {string}
 */
function convertIndentLevelToString(indentLevel: number, tabSize: number): string {
    if (isUsingHardTab()) {
 ...
 */
export function reindentCurrentLine(indentAction: vscode.IndentAction, validPreviousLine: string, currentLine: string): void {
...
function getValidPreviousLine(allLinesArray: string[], currentLine: number): string {
...
function debugEstimateIndentLevel() {
    const editor = vscode.window.activeTextEditor
    if (!editor) { return }
